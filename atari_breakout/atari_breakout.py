...
class Ball:
    ...
    def make_ball(self):
        ...
        :return:
        """
        self.speed = 0.0002 #공의 속도
        self.ball = sphere(color=color.magenta, pos=vector(0, -7, 0), size=vector(0.7, 0.7, 0.7), make_trail=False)
        self.ball.v = vec(1, 1, 0) #공의 이동 방향
    ...
    def ball_movement(self):
        ...
        :return:
        """
        self.ball.pos += self.ball.v * self.speed #기본 움직임 제어
        if self.ball.pos.x + 0.4 >= main.wall.right_wall.pos.x: #오른 벽 부딪힐 시
            self.ball.v.x = -self.ball.v.x + uniform(-0.1, 0.1) #x방향 반대로 + 약간의 무작위 방향성
        if self.ball.pos.x - 0.4 <= main.wall.left_wall.pos.x: #왼 벽 부딪힐 시
            self.ball.v.x = -self.ball.v.x + uniform(-0.1, 0.1) #x방향 반대로 + 약간의 무작위 방향성
        if self.ball.pos.y + 0.4 >= main.wall.upper_wall.pos.y: #윗 벽 부딪힐 시
            self.ball.v.y = -self.ball.v.y + uniform(-0.1, 0.1) #y방향 반대로 + 약간의 무작위 방향성
        if self.ball.pos.y - 0.5 <= main.ball_bar.ball_bar.pos.y and (self.ball.pos.x <= main.ball_bar.ball_bar.pos.x + 2.5 and self.ball.pos.x >= main.ball_bar.ball_bar.pos.x - 2.5): #바와 부딪힐 시
            self.ball.v.y = -self.ball.v.y + uniform(-0.1, 0.1) #y방향 반대로 + 약간의 무작위 방향성
            if self.ball.v.x < 0 and self.ball.pos.x > main.ball_bar.ball_bar.pos.x:
                self.ball.v.x = -self.ball.v.x + uniform(-0.1, 0.1) #왼쪽으로 이동하는 공이 바의 오른쪽 부분(중심 기준)에 맞았을 때는 오른쪽으로 튕겨나가기(원래 게임 환경 적용)
            elif self.ball.v.x > 0 and self.ball.pos.x < main.ball_bar.ball_bar.pos.x:
                self.ball.v.x = -self.ball.v.x + uniform(-0.1, 0.1) #오른쪽으로 이동하는 공이 바의 왼쪽 부분(중심 기준)에 맞았을 때는 오른쪽으로 튕겨나가기(원래 게임 환경 적용)
            else:
                pass
        if self.ball.pos.y <= main.ball_bar.ball_bar.pos.y: #구멍으로 빠질 시
            self.ball.visible = False #공을 안보이게 하고
            self.ball.__del__() #공을 전산에서 삭제(실제로는 남아 있지만 호출 불가능)
        ...
        for block in main.blocks.blocks: #블록과 부딪힐 시
            if self.ball.pos.x > block.pos.x + 1.21 or self.ball.pos.x < block.pos.x - 1.21: #블록의 x좌표 범위내에 없을 경우
                pass
            elif self.ball.pos.y > block.pos.y + 0.71 or self.ball.pos.y < block.pos.y - 0.71: #블록의 y좌표 범위내에 없을 경우
                pass
                ...
                if block.color == vector(0, 1, 0): #색깔이 초록색(1단계)이면 제일 낮은 속도(기본 속도) 유지
                    ...
                    if self.speed < 0.00024: #이미 노란색에 닿은적이 있는지 확인
                        self.speed = 0.00024
                    else:
                        pass
                elif block.color == vector(1, 0.6, 0): #색깔이 주황색(3단계)이면 두번째로 높은 속도로 변경(처음 닿은 이후 계속 유지)
                    if self.speed < 0.00026: #이미 주황색에 닿은적이 있는지 확인
                        self.speed = 0.00026
                    else:
                        pass
                elif block.color == vector(1, 0, 0): #색깔이 빨간색(4단계)이면 제일 높은 속도로 변경(처음 닿은 이후 계속 유지)
                    if self.speed < 0.00028: #이미 빨간색에 닿은적이 있는지 확인
                        self.speed = 0.00028
                    else:
                        pass
                else:
                    pass
                if self.ball.pos.x <= block.pos.x + 1.21 or self.ball.pos.x >= block.pos.x - 1.21: #블록의 윗면이나 아랫면과 닿은 경우(이미 닿은 경우로 위에서 분류했으므로 x범위 안 인지만 확인만 하면 됨)
                    self.ball.v.y = -self.ball.v.y + uniform(-0.1, 0.1) #y방향 반대로 + 약간의 무작위 방향성
                    block.size = vec(0, 0, 0) #블록 삭제(크기 0으로 하는 것으로 대체)
                    main.blocks.blocks.remove(block) #리스트에서 블록 삭제함으로써 다음번 for문에서 확인 안되도록 처리
                else:
                    try: #이미 위에서 삭제 되었을 경우 처리하기 위한 try문
                        if self.ball.pos.y <= block.pos.y + 0.71 or self.ball.pos.y >= block.pos.y - 0.71: #블록의 왼쪽면이나 오른쪽 면과 닿은 경우(이미 닿은 경우로 위에서 분류했으므로 y범위 안 인지만 확인만 하면 됨)
                            self.ball.v.x = -self.ball.v.x + uniform(-0.1, 0.1) #x방향 반대로 + 약간의 무작위 방향성
                            block.size = vec(0, 0, 0) #블록 삭제(크기 0으로 하는 것으로 대체)
                            main.blocks.blocks.remove(block) #리스트에서 블록 삭제함으로써 다음번 for문에서 확인 안되도록 처리
                    except:
...
class Ball_Bar:
    ...
    def make_ball_bar(self):
        ...
        :return: 
        """
        self.ball_bar = box(color=color.blue, size=vector(5, 0.3, 0.1), pos=vector(0, -7.5, 0)) #바 생성
        self.ball_bar.v = vector(0, 0, 0) #이동방향 설정
    ...
    def bar_movement(self):
        ...
        if self.AI_PLAY: #자동 플레이 할때
            if main.ball.ball.pos.x + 2.5 > main.wall.right_wall.pos.x: #오른 벽과 닿았다면(공 기준으로 관리함, 왜냐하면 바는 한번 닿으면 움직이지가 않게 되서)
                ...
                self.ball_bar.pos.x = main.ball.ball.pos.x
        else: #자동 플레이가 아니라면
            self.ball_bar.pos.x += self.ball_bar.v.x * self.bar_speed #기본 움직임 제어
            self.keys = keysdown() #키보드 인식
            if 'left' in self.keys: #왼쪽 화살표키가 눌렸다면
                if self.ball_bar.pos.x - 2.6 <= self.main.wall.left_wall.pos.x: #왼벽에 닿아다면
                    self.ball_bar.v = vector(0, 0, 0) #안 움직이기
                else:
                    self.ball_bar.v = vector(-1, 0, 0) #왼쪽으로 방향 설정
            elif 'right' in self.keys: #오른쪽 화살표키가 눌렸다면
                if self.ball_bar.pos.x + 2.6 >= self.main.wall.right_wall.pos.x: #오른벽에 닿아다면
                    self.ball_bar.v = vector(0, 0, 0) #안 움직이기
                else:
                    self.ball_bar.v = vector(1, 0, 0) #오른쪽으로 방향 설정
            else:
                self.ball_bar.v = vector(0, 0, 0) #아무것도 안눌렸다면 안 움직이기
...
class Wall:
    ...
    def make_wall(self):
        ...
        :return:
        """
        self.left_wall = box(size=vector(0.1, 15, 0.1), pos=vector(-10.1, 0, 0)) #왼 벽
        self.right_wall = box(size=vector(0.1, 15, 0.1), pos=vector(10.1, 0, 0)) #오른 벽
        self.upper_wall = box(size=vector(20.2, 0.1, 0.1), pos=vector(0, 7.5, 0)) #윗 벽
...
class Main:
    ...
    중심 클래스
    """
    def __init__(self, ai_play):
        """
        클래스 Main 세팅
        :param ai_play: -> 자동플레이 여부를 클래스 Ball_Bar에 전달하기 위한 매개변수
        """
        self.start_time = time() #시작하는 시간 기록
        self.end = False #게임이 종료됬는지 여부(이기거나 져서)
        self.monitor = canvas(width=1300, height=800) #화면 설정(사이즈도 설정)
